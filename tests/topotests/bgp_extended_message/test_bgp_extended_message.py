...
def test_bgp_extended_message():
    ...
    test_func = functools.partial(_bgp_extended_message_capability_received, router)
    success, result = topotest.run_and_expect(test_func, None, count=60, wait=0.5)

    assert result is None, 'Failed to see an extended message capability in "{}"'.format(router)
