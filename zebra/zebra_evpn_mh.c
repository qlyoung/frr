...
static void zebra_evpn_es_l2_nh_show_entry(struct zebra_evpn_l2_nh *nh,
		struct vty *vty, json_object *json_array)
{
	if (json_array) {
		json_object *json = NULL;

		json = json_object_new_object();
		json_object_string_add(json, "vtep", inet_ntoa(nh->vtep_ip));
		json_object_int_add(json, "nhId", nh->nh_id);
		json_object_int_add(json, "refCnt", nh->ref_cnt);

		json_object_array_add(json_array, json);
	} else {
		vty_out(vty, "%-16s %-10u %u\n",
				inet_ntoa(nh->vtep_ip), nh->nh_id, nh->ref_cnt);
	}
}

static void zebra_evpn_l2_nh_show_cb(struct hash_bucket *bucket,
		void *ctxt)
{
...
void zebra_evpn_l2_nh_show(struct vty *vty, bool uj)
{
	...
	hash_iterate(zmh_info->nh_ip_table, zebra_evpn_l2_nh_show_cb,
			&wctx);
...
static void zebra_evpn_l2_nh_es_vtep_ref(struct zebra_evpn_es_vtep *es_vtep)
{
	...
	if (!es_vtep->nh) {
		zlog_warn("es %s vtep %s nh ref failed",
			es_vtep->es->esi_str, inet_ntoa(es_vtep->vtep_ip));
		return;
	}
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
		zlog_debug("es %s vtep %s nh %u ref %u",
			es_vtep->es->esi_str, inet_ntoa(es_vtep->vtep_ip),
...
static void zebra_evpn_l2_nh_es_vtep_deref(struct zebra_evpn_es_vtep *es_vtep)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
		zlog_debug("es %s vtep %s nh %u deref %u",
			es_vtep->es->esi_str, inet_ntoa(es_vtep->vtep_ip),
			nh->nh_id, nh->ref_cnt);
...
static void zebra_evpn_mh_uplink_cfg_update(struct zebra_if *zif, bool set)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("mh-uplink-cfg-chg on if %s/%d %s uplinks cfg %u up %u",
...
void zebra_evpn_mh_uplink_oper_update(struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("mh-uplink-oper-chg on if %s/%d %s; uplinks cfg %u up %u",
			zif->ifp->name, zif->ifp->ifindex,
...
void zebra_evpn_mh_init(void)
{
	...
	zmh_info->nh_ip_table = hash_create(zebra_evpn_nh_ip_hash_keymake,
			zebra_evpn_nh_ip_cmp, "l2 NH IP table");
